...
\begin{document}
    	...
    	\begin{tikzpicture}[scale=.7]
		    ...
		    \filldraw (\orig+5*\trans,0) circle (0) node [shift={(-0.4,0.3)}] {$|\psi_i|$};
...
%			\path [->] (\orig+5*\trans,+\del)  edge [bend left=-90] (\orig+1.5*\trans,+\del);
			...
			\filldraw (\orig+5*\trans,\vertspac) circle (\rad) node [shift={(-0.7,0.3)}] {$\sqrt{\lb} |\psi_{i'}|$};
			\filldraw (\orig+8*\trans,\vertspac) circle (\rad); % node [below] {$\ket{\x}$};
		\end{tikzpicture}
