...
%	$C_3 = \lett{ABA}$ \\
%	$C_4 = S(C_3) = }
%	\only<5>{\sub(\A)\sub(\B)\sub(\A)}
%	\only<6>{\lett{AB}\sub(\B)\sub(\A)}
%	\only<7>{\lett{AB}\A\sub(\A)}
%	\only<8>{\lett{AB}\A\lett{AB}}
%	$
\>
...
\begin{itemize}
	...
	{\centering
	\input{img/2_part1/fibonacci_approximant}%
	}
...
\begin{itemize}
	...
	\end{itemize}
	\item \textbf{exact description of the $E=0$ state}
 	\textbf{[Kohmoto \etal{} 87], [Macé \etal{} 17]}
\end{itemize}
\>
\)
\end{frame}
...
\begin{itemize}
	...
	{\centering	
	\input{img/2_part1/field_of_heights}
	}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Red points show such a local power-law, $\alpha = 2|\log \rho|/(3\log \tau)$
		\item \textbf{critical state}
	\end{itemize}
\end{itemize}
\>
\)
...
with
\[
	\om(\beta) = \frac{\sinh^{-1}(1+\cosh(\beta))}{\log(2+\sqrt{5})}
\]
...
{\centering
\includegraphics[width=.75\textwidth]{img/2_part1/dims_fibo}

}


$0 < d_{q>0} < 1$ $\to$ \textbf{multifractal state}

\flushright{[Kohmoto \etal{} 87], [Macé \etal{} 17]}
\>
\)
...
Unbounded fluctuations (non-Pisot inflation factor)
...
{\centering
\includegraphics[width=.7\textwidth]{img/2_part1/height_b3}
...
h(m) \sim m^\alpha
\]
$\alpha = \log 3/\left(3\log(\frac{1+\sqrt{13}}{2})\right)$
