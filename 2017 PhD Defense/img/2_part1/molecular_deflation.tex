...
%		    \draw [rounded corners] (\orig +6*\trans-\del,-\vertsize) rectangle (\orig+7*\trans+\del,\vertsize);
			...
			\foreach \x in {1}
			{
				...
				\filldraw (\orig+1.5*\trans,\x*\vertspac) circle (\rad) node [below] {$\pm t_s$};
				\filldraw (\orig+3.5*\trans,\x*\vertspac) circle (\rad) node [below] {$\pm t_s$};
				\filldraw (\orig+6.5*\trans,\x*\vertspac) circle (\rad) node [below] {$\pm t_s$};
