...
%	\end{itemize}
%
%	\flushleft{In the limit $\rho \ll 1$, $z= \rho/2$, $\zb = \rho^2$ [Niu, Nori 86, Kalugin \etal{} 86]}
%\end{frame}
...
RG $\to$ fractal dimensions of \emph{all} the eigenstates

Eigenstate $\leftrightarrow$ RG path (sequence of molecular and atomic RG steps)

\[
(q-1)D_q(x) = \log \Bigg[ \left( \frac{\lambda(\rho)^q}{\lambda(\rho^q)} \right)^x \left( \frac{\bar{\lambda}(\rho)^q}{\bar{\lambda}(\rho^q)} \right)^{(1-2x)/2} \Bigg]/\log \tau^{-1}
\]
...
\<{7cm}
\centering
\includegraphics[width=.8\textwidth]{img/3_part2/renorm_th_num}

{\ss Analytical and numerical renormalization factors}
\>
\)
